...
export function offer_signer_capability_ (
  ...
  let temp$1, account_resource, expected_auth_key, expected_auth_key__3, proof_challenge, pubkey, pubkey__2, signer_capability_sig, signer_capability_sig__4, source_address;
  ...
  if (($.copy(account_scheme)).eq(($.copy(ED25519_SCHEME)))) {
    ...
    if (($.copy(account_scheme)).eq(($.copy(MULTI_ED25519_SCHEME)))) {
      ...
      if (!Multi_ed25519.signature_verify_strict_t_(signer_capability_sig__4, pubkey__2, proof_challenge, $c, [new SimpleStructTag(SignerCapabilityOfferProofChallengeV2)])) {
        throw $.abortCode(Error.invalid_argument_($.copy(EINVALID_PROOF_OF_KNOWLEDGE), $c));
      }
    }
    else{
